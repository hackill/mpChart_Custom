...
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends
    ...
    protected StaticLayout mNoDataDescSL;
    ...
    protected void init() {
        ...
        mDescPaint.setTextSize(Utils.convertDpToPixel(18f));

        mInfoPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mInfoPaint.setColor(Color.BLACK); // orange
        mInfoPaint.setTextAlign(Align.CENTER);
        mInfoPaint.setTextSize(Utils.convertDpToPixel(18f));

        mNoDataPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mNoDataPaint.setColor(Color.BLACK); // orange
        mNoDataPaint.setTextAlign(Align.CENTER);
        mNoDataPaint.setTextSize(Utils.convertDpToPixel(15f));
    ...
    /**
     * did this chart has data
     *
     * @return true has data, false do not have data
     */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mDataNotSet || mData == null || mData.getYValCount() <= 0) { // check
            ...
            if (!TextUtils.isEmpty(mNoDataTextDescription)) {
                ...
                canvas.drawText(mNoDataText, x, y, mInfoPaint);

                float textOffset = -mInfoPaint.ascent() + mInfoPaint.descent();

                String[] strings = mNoDataTextDescription.split("\n");

                for (String s : strings) {
                    y += textOffset;
                    canvas.drawText(s, x, y, mNoDataPaint);
                }
